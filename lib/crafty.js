...
Crafty.extend({
    ...
    viewport: {
        ...
        follow: (function () {
            ...
            function change() {
                var scale = Crafty.viewport._scale;
                Crafty.viewport.scroll('_x', -(this.x + (this.w / 2) - Math.round((Crafty.viewport.width / 2 / scale) - offx * scale)));
                Crafty.viewport.scroll('_y', -(this.y + (this.h / 2) - Math.round((Crafty.viewport.height / 2 / scale) - offy * scale)));
                Crafty.viewport._clamp();
            }
    ...
    registerEntity: function(e) {
        if (this._registryHoles.length === 0) {
            if (this._registrySize >= this.max_size) {
                throw ("Number of entities exceeds maximum limit (" + this.max_size + ").");
